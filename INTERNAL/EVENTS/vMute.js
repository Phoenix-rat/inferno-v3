...
class PermaBanEvent {
    ...
    async run(member, executor, reason, duration) {
        ...
        try {
            await member.user.send(`**${member.guild.name}** sunucusundaki \`ses kanallarına\` erişimin \`${reason}\` sebebiyle <@${executor}> (\`${executor}\`) tarafından **${duration} dakika** boyunca kısıtlandı! \`${srID}\``);
        } catch (e) {
            console.log(e);
        }
