...
class ChannelUpdate {
    ...
    async run(oldChannel, curChannel) {
        ...
        if ((permission && (permission.count > 0)) || utils.get("root").value().includes(entry.executor.id)) {
            ...
            return client.extention.emit('Logger', 'Guard', entry.executor.id, "CHANNEL_OVERWRITE_CREATE", `${curChannel.name} isimli kanalda izin oluşturdu. Kalan izin sayısı ${permission.count - 1}`);
        }
