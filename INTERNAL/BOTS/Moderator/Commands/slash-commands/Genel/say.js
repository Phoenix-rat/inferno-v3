...
module.exports = class RegistryCommand extends SlashCommand {
    ...
    async run(ctx) {
        ...
        \`•\` Taglı sayısı: \`${guild.members.cache.filter(m => client.config.tag.some(tag => m.user.username.includes(tag)) || m.user.discriminator === client.config.dis).size}\` (${guild.members.cache.filter(m => m.roles.cache.has(roles.get("cmd-crew").value())).size} yetkili)
