...
class RolSeçim extends Component {
    ...
    async run(ctx) {
        ...
        if (!ctx.data.data.values) return await await mentioned.roles.remove(Object.keys(roles.value()).filter(key => key.startsWith("katılımcı_")).map(key => roles.get(key).value()));
