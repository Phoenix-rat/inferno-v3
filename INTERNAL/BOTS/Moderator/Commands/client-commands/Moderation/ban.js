...
class Ban extends Command {
    ...
    async run(client, message, args) {
        ...
        await message.inlineReply(new Discord.MessageEmbed().setDescription(`**${mentioned.user.tag}** (\`${mentioned.user.id}\`) adlı kullanıcı ${message.member} (\`${message.member.user.id}\`) tarafından ${sebep} sebebiyle sunucudan yasaklandı!`).setColor("BLACK"));
