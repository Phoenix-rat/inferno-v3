...
class Say extends Command {
    ...
    async run(client, message, args) {
        ...
        \`•\` Toplam üye: \`${message.guild.memberCount}\` (${message.guild.members.cache.filter(m => m.presence.status !== 'offine').size} online)
        ...
        \`•\` Anlık ses: \`${message.guild.voiceStates.cache.size}\` (${message.guild.voiceStates.cache.filter(v => v.channel && (v.channel.parentID === channels.get("st_public").value())).size} public)
        `).setColor('#7bf3e3'));
    }
}
