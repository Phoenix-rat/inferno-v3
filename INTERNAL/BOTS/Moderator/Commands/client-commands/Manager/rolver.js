...
class Rolver extends Command {
    ...
    async run(client, message, args, data) {
        ...
        let select = args[1]
        if (!select || (select !== 'vip' && select !== 'dj' && select !== 'editör' && select !== 'vokal' && select !== 'ressam' && select !== 'streamer' && select !== 'müzisyen' && select !== 'tasarımcı')) return message.channel.send('Test').then(x => x.delete({ timeout: 5000 }));
        ...
        if(select == "vokal") {
            if(!gavat.roles.cache.has(roles.get("role_vokal").value())) {
                ...
                const vokalembed = new Discord.MessageEmbed().setDescription(`Başarıyla ${roles.get("role_vokal").value()} adlı rolü verdim.`)
                return message.channel.send(vokalembed)
            }
        }
        if(select == "ressam") {
