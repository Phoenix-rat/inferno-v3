...
class RolSeçim extends Component {
    ...
    async run(ctx) {
        ...
        const myRol = guild.roles.cache.get(roles.get("renk_3").value());
        if (mentioned.roles.cache.has(myRol.id)) {
            await mentioned.roles.remove(myRol.id);
            return await ctx.send(`Karayip Yeşili rolü üzerinizden alındı`, {
                ephemeral: true
            });
        } else {
            await mentioned.roles.add(myRol.id);
            return await ctx.send(`Karayip Yeşili rolü üzerinize verildi`, {
                ephemeral: true
            });
        }
