...
class RolSeçim extends Component {
    ...
    async run(ctx) {
        ...
        const myRol = guild.roles.cache.get(roles.get("renk_6").value());
        if (mentioned.roles.cache.has(myRol.id)) {
            await mentioned.roles.remove(myRol.id);
            return await ctx.send(`Somon rolü üzerinizden alındı`, {
                ephemeral: true
            });
        } else {
            await mentioned.roles.add(myRol.id);
            return await ctx.send(`Somon rolü üzerinize verildi`, {
                ephemeral: true
            });
        }
