...
class RolSeçim extends Component {
    ...
    async run(ctx) {
        ...
        const myRol = guild.roles.cache.get(roles.get("renk_2").value());
        if (mentioned.roles.cache.has(myRol.id)) {
            await mentioned.roles.remove(myRol.id);
            return await ctx.send(`\`Turkuaz\` rolü üzerinizden alındı`, {
                ephemeral: true
            });
        } else {
            await mentioned.roles.add(myRol.id);
            return await ctx.send(`\`Turkuaz\` rolü üzerinize verildi`, {
                ephemeral: true
            });
        }
