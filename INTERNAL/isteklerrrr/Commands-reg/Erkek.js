...
exports.execute = async (client, message, args) => {
    ...
    if (!isim && !yaş) {
        ...
        if(!memberData) {
            await kayitData.findOneAndUpdate({_id: member.id}, { $set: { isimgecmisi:  [{ isim: `${isim} | ${yaş}`, type: `Erkek`}]  }}, {upsert:true})
            await kayitData.findOneAndUpdate({_id: message.author.id}, {$inc: {toplamkayit: 1, erkekkayit: 1} }, {upsert:true})
        }
    } else {
        if (member.user.username.includes(config.tag)) {
            ...
            message.react(client.emoji("okey")).catch(() => { })
            if(!memberData) {
                await kayitData.findOneAndUpdate({_id: member.id}, { $set: { isimgecmisi:  [{ isim: `${config.tag} ${isim} | ${yaş}`, type: `Erkek`}]  }}, {upsert:true})
                await kayitData.findOneAndUpdate({_id: message.author.id}, {$inc: {toplamkayit: 1, erkekkayit: 1} }, {upsert:true})
            }
        } else {
            ...
            message.react(client.emoji("okey")).catch(() => { })
            if(!memberData) {
                await kayitData.findOneAndUpdate({_id: member.id}, { $set: { isimgecmisi:  [{ isim: `${config.untag} ${isim} | ${yaş}`, type: `Erkek`}]  }}, {upsert:true})
                await kayitData.findOneAndUpdate({_id: message.author.id}, {$inc: {toplamkayit: 1, erkekkayit: 1} }, {upsert:true})
            }
        }
    }
