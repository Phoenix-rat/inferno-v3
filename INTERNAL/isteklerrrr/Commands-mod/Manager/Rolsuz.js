...
module.exports.execute = async (client, message, args) => {
  ...
  if(rolsuz.size == 0) return message.inlineReply(`\`\`\`Sunucumuzda rolsüz üye bulunmamakta.\`\`\``)

  await rolsuz.forEach(member => {
      member.roles.add(client.role("regUnreg")).catch(() => {})
  });

  await message.inlineReply(`\`\`\`Herhangi bir rolü olmayan ${rolsuz.size} kişiye kayıtsız rolü verildi.\`\`\``)
